...
@section('content')
    ...
    <div class="detail-header" style="margin-top: 12px;">
        ...
        <div class="row-actions">
            <a href="{{ route('warehouses.edit', $warehouse) }}" class="btn btn-secondary"><i class="fa-regular fa-pen-to-square"></i> Edit</a>
            <form action="{{ route('warehouses.destroy', $warehouse) }}" method="POST" style="display:inline;">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger-outline" onclick="return confirm('Delete this warehouse?')"><i class="fa-regular fa-trash-can"></i> Delete</button>
            </form>
        </div>
    </div>
    ...
    <div class="stat-grid section">
        <div class="stat-card">
            ...
            <div class="stat-value">0</div><div class="stat-label">Products Stored</div>
        </div>
        <div class="stat-card">
            <div class="stat-card-top"><div class="stat-icon green"><i class="fa-solid fa-boxes-stacked"></i></div></div>
            <div class="stat-value">0</div><div class="stat-label">Total Stock Quantity</div>
        </div>
        <div class="stat-card">
            <div class="stat-card-top"><div class="stat-icon gray"><i class="fa-solid fa-location-dot"></i></div></div>
            <div class="stat-value" style="font-size:16px;">{{ $warehouse->location ?? 'N/A' }}</div><div class="stat-label">Location</div>
        </div>
    </div>
