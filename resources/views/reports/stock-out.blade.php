...
@section('subtitle', 'Completed stock issue lines')
        ...
        <div class="card-body" style="padding-bottom: 0;">
            <form action="{{ route('reports.stock-out') }}" method="GET" class="filters-bar">
                ...
                @if (request()->hasAny(['distributor_id', 'warehouse_id', 'product_id', 'date_from', 'date_to']))
                    <a href="{{ route('reports.stock-out') }}" class="btn btn-secondary btn-sm">Clear</a>
                @endif
            </form>
        </div>
                ...
                <tbody>
                    @forelse ($stockOutItems as $item)
                        <tr>
                            ...
                            <td class="cell-muted">{{ $item->stockOut->issue_date->format('d M Y') }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" style="text-align: center; padding: 40px;">No completed stock-out lines found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
