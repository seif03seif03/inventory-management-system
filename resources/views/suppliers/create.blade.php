...
@section('subtitle', __('Register a new supplier'))

@section('content')

    <div class="breadcrumb">
        <a href="{{ url('/suppliers') }}">{{ __('Suppliers') }}</a> <i class="fa-solid fa-chevron-right" style="font-size:9px"></i> <span>{{ __('Add Supplier') }}</span>
    </div>

    <form action="{{ route('suppliers.store') }}" method="POST" class="card section" style="margin-top: 12px;">
        @csrf
        ...
        <div class="card-body">
            <div class="form-grid">
                <div class="form-group full">
                    ...
                    <input type="text" name="name" class="form-control" value="{{ old('name') }}" placeholder="e.g. Ahmed Hassan" required>
                    @error('name')
        ...
        <div class="card-body" style="padding-top:0;">
            <div class="form-actions">
                ...
                <button type="submit" class="btn btn-primary"><i class="fa-solid fa-check"></i> {{ __('Save Supplier') }}</button>
            </div>
        </div>
    </form>
